...
def lyotstop(wf, conf, RAVC):
    ...
    if conf['PHASE_APODIZER_FILE'] == 0:
        ...
        LS_phase_apodizer_file = fits.getdata(input_dir+'/'+conf['PHASE_APODIZER_FILE'])
    LS = conf['LYOT_STOP']
...
#        scaling_factor = float(npupil)/float(pupil_pixels) ## scaling factor between the fits file size and the pupil size of the simulation
        ...
        apodizer_scale = cv2.resize(LS_phase_apodizer_file.astype(np.float32), (0,0), fx=scaling_factor, fy=scaling_factor, interpolation=cv2.INTER_LINEAR) # scale the pupil to the pupil size of the simualtions
        if (Debug_print==True):
