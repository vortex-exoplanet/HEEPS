...
cube = vip_hci.fits.open_fits(out_dir + 'test_PSF_cube_VC.fits')          # coronagraphic PSFs cube (non-normalized)


length,x,y = cube.shape

psf= psf/sum(sum(psf_noMask))                                                  # off-axis PSF normalized wrt the total flux of the non-coronagraphic PSF
cube = cube/sum(sum(psf_noMask))                                               # cube normalized wrt the total flux of the non-coronagraphic PSF

angs = np.linspace(-160,160,length)

exp_t = 3600/length                                                            # exposure time in sec for 1 hr sequence
...
psc_metis = 0.005 #25 # plate scale (arcsec/pix) of the METIS L-band detector
